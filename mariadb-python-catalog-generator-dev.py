...
# project_class = 0 are PreTeXt projects, 1 is Converting, 2 is In Development
    for project_class in range(3):
        if project_class == 0:
            ...
            cursor.execute("SELECT * from projects WHERE (project_phase_id = '4') ORDER BY project_title")
        else:
            cursor.execute("SELECT * from projects WHERE (project_phase_id = '1') ORDER BY project_title")
...
# Project being Converted or In Development
                if current_project_count == 0:
                    if project_class == 1:
                        output.write("\n      <div class=\"category\">\n")
                        output.write("        <span class=\"title\">Mature, Converting to PreTeXt</span>\n\n")
                    else:
                        output.write("\n      <div class=\"category\">\n")
                        output.write("        <span class=\"title\">In Development</span>\n\n")
...
# Assemble lists of coauthors
            ...
            cursor.execute("SELECT * FROM coauthors WHERE coauthors_project_id=?",(coauthorXMLid,))
...
##Blurb section
            output.write("\n          <div class=\"blurb\">\n             <p>\n")
            outstring = "{}\n"
            output.write(outstring.format(project_data[current_project_count][18]))
            ...
            output.write(outstring.format(project_data[current_project_count][19]))
...
# Badges
            ...
            cursor.execute("SELECT * from recognition WHERE recognition_code=?",(project_recognition_code,))
...
# No space between <img> tags to keep badges tight
            ...
            cursor.execute("SELECT * from licenses WHERE license_code=?",(project_license_code,))
...
## Catalog statistics:
    ...
    for l_id in license_tags:
        cursor.execute("SELECT project_xml_id FROM projects WHERE project_license_code = ?", (l_id,))
        records = cursor.fetchall()
